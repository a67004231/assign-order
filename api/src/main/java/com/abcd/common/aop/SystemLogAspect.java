...
import com.abcd.common.utils.ThreadPoolUtil;
import com.abcd.modules.base.entity.Log;
import com.abcd.modules.base.entity.elasticsearch.EsLog;
import com.abcd.modules.base.service.LogService;
import com.abcd.modules.base.service.elasticsearch.EsLogService;
...
//    @Pointcut("execution(* *..controller..*Controller*.*(..))")
    @Pointcut("@annotation(com.ryhy.common.annotation.SystemLog)")
...
//                    log.setIpInfo(joinPoint.getArgs());
                    //请求开始时间
                    Date logStartTime = beginTimeThreadLocal.get();
...
// 在方法执行完结后打印返回内容
    ...
    public void methodAfterReturing(Object o) {
        try {
            ...
            sysLog.setResponseParam(jsonObject.toJSONString(o));
            sysLog.setUpdateBy("API_RESPONSE");
            //调用线程保存至ES
//            ThreadPoolUtil.getPool().execute(new UpdateSystemLogThread(sysLog, logService));
     ...
     * 更新日志至数据库
     */
    private static class UpdateSystemLogThread implements Runnable {

        private Log log;
        private LogService logService;

        public UpdateSystemLogThread(Log esLog, LogService logService) {
            this.log = esLog;
            this.logService = logService;
        }
